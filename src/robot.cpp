...
void uninit_sys() {
	...
	// Turn off LEDs
	P8.digitalSetValue(LED1_PIN, true);
	P8.digitalSetValue(LED2_PIN, true);
}

// Init the robot variables
void robot_init() {
	// Change start button pin to input
	P9.pinSetMux(START_BUTTON_PIN, MUX_7, PULLUP_ENABLED, PULLUP, INPUT_PIN);
	P9.pinSetMux(PAUSE_BUTTON_PIN, MUX_7, PULLUP_ENABLED, PULLUP, INPUT_PIN);

	P9.pinExport(START_BUTTON_PIN);
	P9.pinExport(PAUSE_BUTTON_PIN);

	P9.digitalSetDirection(START_BUTTON_PIN, INPUT_PIN);
	P9.digitalSetDirection(PAUSE_BUTTON_PIN, INPUT_PIN);

	// Change LEDs pins to output
	P8.pinSetMux(LED1_PIN, MUX_7, PULLUP_DISABLED, PULLDOWN, OUTPUT_PIN);
	P8.pinSetMux(LED2_PIN, MUX_7, PULLUP_DISABLED, PULLDOWN, OUTPUT_PIN);

	P8.pinExport(LED1_PIN);
	P8.pinExport(LED2_PIN);

	P8.digitalSetDirection(LED1_PIN, OUTPUT_PIN);
	P8.digitalSetDirection(LED2_PIN, OUTPUT_PIN);

	P8.digitalSetValue(LED1_PIN, 1);
	P8.digitalSetValue(LED2_PIN, 1);
}
...
void background() {
	...
	if (current_time > prev_time) {
		...
		case RBS_WAIT_FOR_START_CMD:
			...
			P8.digitalSetValue(LED1_PIN, led);
			P8.digitalSetValue(LED2_PIN, true);
		...
		case RBS_RUNNING:
			// Adjust LEDs
			P8.digitalSetValue(LED1_PIN, false);
			P8.digitalSetValue(LED2_PIN, true);
		...
		case RBS_FINISHED:
			...
			P8.digitalSetValue(LED1_PIN, led);
			P8.digitalSetValue(LED2_PIN, !led);
...
void robot_run(union sigval arg) {
	...
	case RBS_WAIT_FOR_START_CMD:
		...
		if ((P9.digitalGetValue(START_BUTTON_PIN) == false) or (prgm_vars.disableStartButton == true)) {
			// Set starting point
	...
	case RBS_RUNNING:
		if (current_waypoint == route.last()) {
			...
			if (P9.digitalGetValue(PAUSE_BUTTON_PIN) == false) {
	...
	case RBS_PAUSED:
		...
		if (P9.digitalGetValue(PAUSE_BUTTON_PIN) == true) {
	...
	case RBS_FINISHED:
		...
		if (P9.digitalGetValue(START_BUTTON_PIN) == false) {
			// Update start delay time to new value
			start_delay_time = current_time + prgm_vars.startDelayPeriod;

			// Set robot to delay mode
			current_state.state = RBS_START_DELAY;
		} else if (P9.digitalGetValue(PAUSE_BUTTON_PIN) == false) {
