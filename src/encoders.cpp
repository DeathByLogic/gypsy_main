...
void encoders_init() {
	...
	P8.pinExport(ENC_RIGHT_A_PIN);
	P8.pinExport(ENC_RIGHT_B_PIN);
	P8.pinExport(ENC_LEFT_A_PIN);
	P8.pinExport(ENC_LEFT_B_PIN);

	// Change encoder pins to input
	P8.digitalSetDirection(ENC_RIGHT_A_PIN, INPUT_PIN);
	P8.digitalSetDirection(ENC_RIGHT_B_PIN, INPUT_PIN);
	P8.digitalSetDirection(ENC_LEFT_A_PIN, INPUT_PIN);
	P8.digitalSetDirection(ENC_LEFT_B_PIN, INPUT_PIN);

	// Attach interrupt pins to encoder functions
	P8.digitalSetEdge(ENC_RIGHT_A_PIN, BOTH_EDGES);
	P8.digitalSetEdge(ENC_RIGHT_B_PIN, BOTH_EDGES);
	P8.digitalSetEdge(ENC_LEFT_A_PIN, BOTH_EDGES);
	P8.digitalSetEdge(ENC_LEFT_B_PIN, BOTH_EDGES);
...
void *encoder_thread(void *arg) {
	...
	enc_int[0].fd = P8.pinOpen(ENC_RIGHT_A_PIN);
	enc_int[1].fd = P8.pinOpen(ENC_RIGHT_B_PIN);
	enc_int[2].fd = P8.pinOpen(ENC_LEFT_A_PIN);
	enc_int[3].fd = P8.pinOpen(ENC_LEFT_B_PIN);
	...
	while (true) {
		...
		if ((rc = poll(enc_int, ENC_POLL_COUNT, ENC_POLL_TIMEOUT)) < 0) {
			...
			if ((enc_int[0].revents & POLLPRI) or (enc_int[1].revents & POLLPRI)) {
				...
				P8.fdigitalGetValue(enc_int[0].fd, &enc_value[0]);
				P8.fdigitalGetValue(enc_int[1].fd, &enc_value[1]);
			...
			if ((enc_int[2].revents & POLLPRI) or (enc_int[3].revents & POLLPRI)) {
				// Read in encoder values
				P8.fdigitalGetValue(enc_int[2].fd, &enc_value[2]);
				P8.fdigitalGetValue(enc_int[3].fd, &enc_value[3]);
